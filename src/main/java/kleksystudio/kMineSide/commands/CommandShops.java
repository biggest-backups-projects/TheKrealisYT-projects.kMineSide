...
public class CommandShops implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if(s instanceof Player && !s.equals(null)) {
            ...
            List<String> list = plugin.getConfig().getStringList("messages.warps.shops");
            for (String h : list) {
                p.sendMessage(ServerUtils.s(h));
            }
            p.teleport(new Location(Bukkit.getServer().getWorld(plugin.getConfig().getString("settings.worlds.spawn", "spawn")), 10.5F, 50F, -16.5F, -179.9F, 0));
            return true;
        }
